...
<script type="text/javascript">
	...
	connButton.onclick = function() {
		if (!demo.socket)
			...
			getMessage('websocket already exists .');
	};
	discButton.onclick = function() {
		if (demo.socket)
			demo.socket.close();
		else
			getMessage('websocket is not found .');
	};
...
<body>
				...
				<div class="main clearfix">
					<div class="column">
						...
						<button id="notification-trigger" class="progress-button"
							onclick="getMessage()">
							<span class="content">Message</span> <span class="progress"></span>
						</button>
						<button id="buttonConnect" onclick="connectServer()">Connect
							to server</button>
					</div>
					<div id="post">
						<%@ page import="com.web.*"%>
	...
	CPost cp = new CPost();%>
						<%
							...
							try {
								jpage = new JPage(Long.parseLong(id));
							} catch (NumberFormatException e) {

								//page not existed page.

							}
							List posts = jpage.getPosts();
							for (String[] s : posts) {
								out.println("<a href=\"user.jsp?id=" + s[0] + "\">" + s[1]
										+ "</a>:" + "</br>");
								out.println("<p>" + s[2] + "</p>");
