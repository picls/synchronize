...
public class LoginAction extends ActionSupport implements ServletRequestAware,
	...
	public String execute() throws Exception {
		...
		JAccount jaccount = new JAccount(username);
		if (jaccount.login(password)) {
			FORWARD = "success";
	...
	/*
	 * @Override // 正常登录 public String execute() throws Exception {
	 * System.out.println(username + "\t" + password + "\t" + userCookie);
	 * String username = getUsername().trim(); if (username != null &&
	 * !"".equals(username) && password != null && !"".equals(password)) { User
	 * user = userDao.checkUser(username, password); System.out.println(user);
	 * if (user != null) { // 判断是否要添加到cookie中 if (userCookie) { Cookie cookie =
	 * cookieUtils.addCookie(user); response.addCookie(cookie);//
	 * 添加cookie到response中 } // 2.将user 设置到session中 session.put(USER_SESSION,
	 * user); return SUCCESS; } } this.addFieldError("username", "用户名或密码错误!");
	 * return "login"; }
	 */
